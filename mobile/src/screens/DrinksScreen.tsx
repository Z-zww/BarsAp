...
export default function DrinksScreen() {
  ...
  const load = useCallback(async (q?: string) => {
    ...
    setError('');
    try {
      const list: any = await api.drinks(q ? { q, limit: 50 } : { limit: 50 });
      setDrinks(list);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={drinks}
        keyExtractor={(d) => d.id}
        renderItem={({ item }) => <DrinkCard drink={item} onPress={() => navigation.navigate('DrinkDetail', { id: item.id })} />}
        contentContainerStyle={styles.list}
        ListEmptyComponent={<Text style={styles.empty}>{loading ? '加载中…' : '暂无酒品'}</Text>}
      />
    </View>
  );
}
